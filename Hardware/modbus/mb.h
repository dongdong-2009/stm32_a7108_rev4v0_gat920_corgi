...
/*! \ingroup modbus
 *  交代 If register should be written or read.
 *
 * 这个值被传递给回调函数,无论是读或写的寄存器值
 * 写意味着应用寄存器应该被更新，读意味着modbus
 *协议栈需要知道当前寄存器的值。
 ...
 * init functions of the porting layer to prepare the hardware.
 *请注意，接收器仍然禁用，没有的Modbus帧被处理，直到eMBEnable（）被调用。
...
eMBErrorCode    eMBInit( eMBMode eMode, UCHAR ucSlaveAddress,
                         UCHAR ucPort, UCHAR ucBaudRate, eMBParity eParity );
 ...
 * is disabled. 
 * 此函数disable ModBus 协议栈和释放所有邮件资源，当协议栈Disable
 * 的时候才能被调用。
